...
void check(const std::tuple<std::string, std::string>& Tuple, std::unordered_map<std::string, int>& cards1, std::unordered_map<std::string, int>& cards2, int& point1, int& point2) {
	if (std::get<0>(Tuple) == "king") {
		if (std::get<1>(Tuple) == "king") {
			cards1["king"] -= 1;
			cards2["king"] -= 1;
		} else if (std::get<1>(Tuple) == "servant") {
			point1 += 1;
			cards1["king"] -= 1;
			cards2["servant"] -= 1;
		} else if (std::get<1>(Tuple) == "slave") {
			point2 += 1;
			cards1["king"] -= 1;
			cards2["slave"] -= 1;
		}
	} else if (std::get<0>(Tuple) == "servant") {
		if (std::get<1>(Tuple) == "king") {
			point2 += 1;
			cards1["servant"] -= 1;
			cards2["king"] -= 1;
	...
	if (auto it = cards1.find("king"); it != cards1.end() && cards1["king"] <= 0) {
		cards1.erase(it);
	} else if (auto it = cards1.find("servant"); it != cards1.end() && cards1["servant"] <= 0) {
		cards1.erase(it);
	} else if (auto it = cards1.find("slave"); it != cards1.end() && cards1["slave"] <= 0) {
		cards1.erase(it);
	}
	
	if (auto it = cards2.find("king"); it != cards2.end() && cards2["king"] <= 0) {
		cards2.erase(it);
	} else if (auto it = cards2.find("servant"); it != cards2.end() && cards2["servant"] <= 0) {
		cards2.erase(it);
	} else if (auto it = cards2.find("slave"); it != cards2.end() && cards2["slave"] <= 0) {
		cards2.erase(it);
	}
}
...
int main() {
	...
	//auto TDcards = iter::combinations(std::vector<std::string>{"king", "servant", "slave"}, 2);
	constexpr size_t lenth = 36;
	auto TDcards = iter::combinations_with_replacement(std::vector<std::string>{"king", "servant", "slave"}, 2);
	...
	for (const auto& i : defaultCards) {
		cards[num] = std::make_tuple(std::unordered_map<std::string, int>{{"king", 2 + count(i[0], std::string("king"))}, {"servant", 2 + count(i[0], std::string("servant"))}, {"slave", 2 + count(i[0], std::string("slave"))}}, std::unordered_map<std::string, int>{{"king", 2 + count(i[1], std::string("king"))}, {"servant", 2 + count(i[1], std::string("servant"))}, {"slave", 2 + count(i[1], std::string("slave"))}}, std::array<std::string, 4>{i[0][0], i[0][1], i[1][0], i[1][1]});
		num++;
	}
	...
	for (const auto& card : cards) {
		//iterate++;
		auto positions1 = iter::product(std::vector<std::string>{"king", "servant", "slave"}, std::vector<std::string>{"king", "servant", "slave"});
